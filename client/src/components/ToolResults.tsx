...
import { X402PaymentRequired, X402PaymentResponse } from "./X402PaymentInfo";
...
const ToolResults = ({
  ...
  if ("content" in toolResult) {
    ...
    // x402: Detect payment-required responses and payment settlement metadata
    const x402PaymentRequired = extractPaymentRequiredFromResult({
      content: structuredResult.content as Array<{
        type: string;
        text?: string;
        [key: string]: unknown;
      }>,
      isError,
      structuredContent: structuredResult.structuredContent as
        | Record<string, unknown>
        | undefined,
    });
    const x402PaymentResponse = extractPaymentResponseFromMeta(
      structuredResult._meta as Record<string, unknown> | undefined,
    );
    ...
    return (
      <>
        ...
        {x402PaymentRequired && (
          <div className="mb-4">
            <X402PaymentRequired paymentRequired={x402PaymentRequired} />
          </div>
        )}
        {x402PaymentResponse && (
          <div className="mb-4">
            <X402PaymentResponse settleResponse={x402PaymentResponse} />
          </div>
        )}
        {structuredResult.structuredContent && (
          <div className="mb-4">
